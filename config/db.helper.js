...
const Apartments = require('../src/models/apartments.model');
const Buildings = require('../src/models/buildings.model');
const Contractors = require('../src/models/contractors.model');
const Contracts = require('../src/models/contracts.model');
const Cotisations = require('../src/models/cotisations.model');
const Depenses = require('../src/models/depenses.model');
const Devis = require('../src/models/devis.model');
const Emails = require('../src/models/emails.model');
const Missions = require('../src/models/missions.model');
const Notifications = require('../src/models/notifications.model');
const OwnershipShares = require('../src/models/ownershipShares.model'); // Added
const PartiesCommunes = require('../src/models/partiesCommune.model');
const Permissions = require('../src/models/permissions.model');
const Prestataires = require('../src/models/prestataires.model'); // Note: Seems redundant with Contractors
const ReserveFunds = require('../src/models/reserveFunds.model'); // Added
const Roles = require('../src/models/roles.model');
const ServiceRequests = require('../src/models/serviceRequests.model');
const Services = require('../src/models/services.model');
const Sessions = require('../src/models/sessions.model');
const Sites = require('../src/models/sites.model');
...
const Votes = require('../src/models/votes.model'); // Added
...
const createInitialPermissions = async (packs, packName) => {
  ...
  const selectedPack = packs.find(pack => pack.packName === packName);
  if (!selectedPack) {
    throw new Error(`'${packName}' pack not found during permission creation!`);
  }

  const permissionsToCreate = permissionsGenerator(
    { id: selectedPack.id, name: selectedPack.packName, type: "packs" },
 ...
 * @throws {Error} If the Administration pack is not found or user creation fails.
 */
const createAdminUser = async (packs) => {
    console.log("Step 3: Creating Initial Admin User...");
    const adminPack = packs.find(pack => pack.packName === 'Administration');
    if (!adminPack) {
        throw new Error("Administration pack not found for admin user creation!");
    }
...
const createInitialData = async () => {
  ...
  try {
    ...
    const [initPermissions, selectedPack] = await createInitialPermissions(initPacks, 'Syndicate');
